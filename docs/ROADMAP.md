...
Status: in progress — design-review package revised per the **seven** review items (removed the fictitious standalone `load_driver` step — the Wintun driver is installed/loaded **inside `WintunCreateAdapter`** and there is no separate driver-load op; fixed enable order so routes/DNS are always written after adapter creation (verify → elevate/bootstrap → snapshot → create adapter → pin LUID → journal intent → apply → mihomo open → probe); chose a **single OS network-config owner** — **Option A** where the helper is the sole route/DNS/interface modifier, mihomo's runtime config has `auto-route:false`/`auto-detect-interface:false`/`dns-hijack:false`, and the main process generates a typed `DesiredNetworkState`; rewrote the elevation/IPC bootstrap as an executable **elevation-COM server** sequence (`runas` cannot inherit handles); fixed the key lifecycle (zero only `launchSecret` post-handshake, retain `sessionKey` to channel close/task end, `RtlSecureZeroMemory` on all paths) and the canonical length-prefixed MAC; fixed `dnsSets`/`NET_LUID`/`requestId` types; expanded the test matrix) and **design re-review requested** (round 3); **implementation gate still NOT met** (design-review sign-off and separate owner authorization still required, plus D4/D5, G1 (mihomo reusing the helper-created adapter — an **unproven hypothesis** to be proven by a real Windows integration test) and the certificate-provider decision).

- [~] Write and approve helper/privilege threat model. (draft → `docs/helper-threat-model.md`; revised per the **seven** review items: device model = driver installed/loaded inside `WintunCreateAdapter`, no separate load step; IPC control C3 rewritten as the elevation-COM bootstrap; single OS-config owner D6 (Option A); G1 outlined as an unproven handoff hypothesis; approval pending)
- [~] Define install, upgrade, rollback and uninstall behavior. (draft → `docs/helper-install-upgrade-rollback.md`; revised per the **seven** review items: adapter/driver created at first enable via `WintunCreateAdapter`, no `load_driver` op; routes/DNS always after adapter creation; mihomo adds no route/DNS; uninstall never deletes a shipped/pre-existing driver; expanded test map; approval pending)
- [~] Design review package. (draft → `docs/helper-design.md`: shared TUN contract, elevation-COM IPC/envelope + key lifecycle + canonical MAC, `WintunCreateAdapter`-on-demand model + **G1** (mihomo reuses the helper-created adapter, unproven), single OS-config owner `DesiredNetworkState`, fixed types, per-item snapshot/restore + reverse-journal-order recovery, unified TUN state machine, config-gating design, renderer intent-only contract, module layout, expanded test/evidence matrix; D4/D5, G1 + certificate provider still open — **implementation gate NOT met**)
