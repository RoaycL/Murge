...
Status: in progress — design-review package revised to **rev.4** per the **round-3** review items: (1) Wintun ABI corrected verbatim from the official `wintun.h` (pinned **0.14.1**, per-arch DLL, `WINAPI`, recorded exports/header source) — `WintunCreateAdapter(Name, TunnelType, RequestedGUID) -> handle` with the **app-supplied `RequestedGUID`** as a stable identity, `WintunGetAdapterLUID(h,&luid)`, `WintunStartSession(h,capacity)` creates the session, and `WintunEndSession`/`WintunCloseAdapter`/`WintunDeleteAdapter` are distinct (the earlier "Wintun has no RequestedGUID parameter" assertion **withdrawn**; no `WintunFreeAdapter`); (2) elevation rewritten to the official **COM Elevation Moniker** (`CoGetObject("Elevation:Administrator!new:{CLSID}", BIND_OPTS3, ...)`, full HKLM `CLSID`/`Elevation`/`LocalServer32`+`ServerExecutable`/`AppID`/`LaunchPermission`/`AccessPermission`, `RunAs = "Interactive User"`, `CoInitializeSecurity`/`CoSetProxyBlanket` authn/impersonation/packet-privacy); (3) client PID fixed to **`RPC_CALL_ATTRIBUTES_V2.ClientPID`** + `RPC_QUERY_CLIENT_PID`, only local **`ncalrpc`** accepted, PID used only to open/validate the process object — never identity by itself; (4) a **true write-ahead journal** (`CREATE_ADAPTER/PREPARED` fsync'd before `WintunCreateAdapter`, `APPLIED` after, each route/DNS op `PREPARED`→mutate→`APPLIED`, recovery reconciles PREPARED-but-unknown by enumerating the product adapter/current OS state); (5) **explicit adapter deletion** (`WintunDeleteAdapter`, `RebootRequired`→`delete-pending`, product-owned only, `DELETE_ADAPTER/RECONCILED`, no automatic-delete-on-session-close claim); (6) a **per-activation, short-lived elevated COM server** bound to the first verified client, rejecting all other clients (including a second Murge process with an identical binary) and exiting on complete/cancel/timeout; (7) **G1 kept as a hard pre-implementation gate** proven by a **minimal one-shot disposable Windows probe** (create adapter → verify same GUID/LUID → delete + restore, not a full Phase 9 implementation). The round-1/round-2 fixes (no `load_driver`; enable order; single OS-config owner Option A; elevation-moniker bootstrap replacing the inherited-handle scheme; key lifecycle + canonical MAC; `dnsSets`/`NET_LUID`/`requestId` types; test matrix) are retained (§0.1). **Design re-review requested (round 4); implementation gate still NOT met** (design-review sign-off + separate owner authorization still required, plus D4/D5, G1 (unproven, to be proven by the one-shot probe) and the certificate-provider decision).

- [~] Write and approve helper/privilege threat model. (draft → `docs/helper-threat-model.md`; revised per the **round-3** review items: device model = driver installed/loaded inside `WintunCreateAdapter(Name, TunnelType, RequestedGUID)`, no separate load step; C3 rewritten as the **COM elevation-moniker** bootstrap with `CoGetObject("Elevation:Administrator!new:{CLSID}")`, `RunAs = "Interactive User"`, `ClientPID` + `RPC_QUERY_CLIENT_PID` + `ncalrpc`-only, and the **per-activation single-client server** rejections; C4 updated to a **write-ahead** journal (PREPARED→mutate→APPLIED); C5 updated to explicit `WintunDeleteAdapter`/RebootRequired + product-owned-only deletion (no automatic-delete claim); single OS-config owner D6 (Option A); G1 outlined as an unproven handoff hypothesis to be proven by a one-shot probe; approval pending)
- [~] Define install, upgrade, rollback and uninstall behavior. (draft → `docs/helper-install-upgrade-rollback.md`; revised per the **round-3** review items: adapter/driver created at first enable via the corrected `WintunCreateAdapter(...RequestedGUID)` ABI, no `load_driver` op; **write-ahead** enable order (PREPARED→APPLIED) with explicit `DELETE_ADAPTER/PREPARED`→`WintunDeleteAdapter`→`RECONCILED` on disable (reboot-aware `delete-pending`); mihomo adds no route/DNS; uninstall never deletes a shipped/pre-existing driver **or a pre-existing adapter**; expanded test map (G1 one-shot probe, second-Murge-instance race, WAL crash boundaries, adapter-deletion/reboot); approval pending)
- [~] Design review package. (draft → `docs/helper-design.md`: shared TUN contract, §3.0 **pinned Wintun 0.14.1 ABI** (correct `WintunCreateAdapter`/`WintunGetAdapterLUID`/`WintunStartSession` + distinct teardown semantics), §3.2 creation with `RequestedGUID` identity, §3.3 **explicit adapter deletion**, elevation-moniker IPC/envelope + `ClientPID` + key lifecycle + canonical MAC, **write-ahead journal** + reconcile-PREPARED-but-unknown (§8.3–§8.5), **per-activation short-lived server** (§5.5), single OS-config owner `DesiredNetworkState`, fixed types, unified TUN state machine, config-gating design, renderer intent-only contract, module layout, expanded test/evidence matrix (G1 probe, second-instance race, WAL boundaries, delete/reboot); D4/D5, G1 + certificate provider still open — **implementation gate NOT met**)
