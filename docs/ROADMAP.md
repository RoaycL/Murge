...
and the Windows job is now green end-to-end including the fault-injection step).
...
> Step A (resolver, download/verify, config generator, config store + unit tests and the gated real-kernel integration test) is implemented and green in CI. Step B (Win x64 real mihomo) has executed on the disposable Windows CI job and the safety hardening is landed: the config store never leaks a workspace child on failure, the job's `if: always()` finally step plus a dedicated fault-injection step both run the shared `scripts/kernel-watchdog-cleanup.mjs` (strict evidence schema, exact-name matching, `binaryPath`-mismatch gating, and symlink/junction/real-path escape protection), and the enriched runtime evidence (binary path, version, PID, listener host:port, `/version` result, network-diff PASS) is captured in the `kernel-real-evidence` artifact. Phase 7 remains **in progress** for two owner-facing items only: Windows arm64 is done or explicitly deferred, and the production lifecycle wired to an explicit user-triggered action. Real mihomo execution stays on the disposable Windows CI / disposable VM; the current host never launches the binary.
