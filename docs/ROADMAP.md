...
迁移为社区式**单 mihomo 内核**：系统代理与 TUN 共用同一个 mihomo（TUN 开启时以特权
方式重启该内核并注入 `tun` 配置），数据面始终读取该单内核控制器。删除"安全直连内核"
概念；系统代理与 TUN 不再互斥，逻辑内核在 TUN 切换期间保持运行；仅显式 `kernel:stop`
会恢复系统代理并停止一切。系统代理 `enable()` 修复了端口跨会话重分配后的陈旧自有
bundle 误判冲突问题。
