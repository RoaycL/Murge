...
- [~] System tray menu and state synchronization. (The main-process `TrayController` is implemented and wired: show/focus, verified kernel phase, serialized start/stop, quit, close-to-tray and idempotent teardown. It consumes the ordered kernel gateway, so tray stop preserves the Phase 8 proxy-before-kernel safety ordering and never toggles optimistically. The Electron adapter and packaged icon are included; controller behavior is unit-tested. The packaged `--hidden`/native `Tray` probe now lives in the manual `windows-gui-smoke` workflow and requires an interactive self-hosted Windows runner; it has not been proven merely by the hosted packaging job. Final notification-area rendering and Explorer restart recovery remain pending on a Windows VM/physical machine.)
- [~] Start-on-login with explicit user opt-in. (A serial `StartupService`, strict boolean IPC, preload API and Settings → General switch are implemented. The default path only reads state and never writes; only an explicit user toggle invokes the OS adapter, followed by read-after-write confirmation with divergence/error shown without optimistic UI. Windows login launches with `--hidden`, creating only the tray/UI process — never the kernel, proxy or TUN; the installed-artifact `--hidden-smoke` path is retained in the manual interactive `windows-gui-smoke` workflow without changing login registration. Non-Windows fails closed as unsupported. Unit coverage proves default-off/no-write, explicit enable, divergence and unsupported behavior; final real workflow plus sign-out/sign-in Task Manager lifecycle evidence remain pending.)
...
- [x] Accessibility contract review for the frozen RC surface. (Supported form fields have accessible names independent of placeholders; dynamic errors/status use alert/live semantics; process/device selection exposes pressed state; static segmented controls expose their selected state; disabled placeholder controls are labelled. These semantic-only changes preserve the approved geometry. Final screen-reader and Windows high-contrast human validation remains part of RC acceptance.)
...
- [~] Run clean-install, upgrade and uninstall matrices. (Hosted CI exercises clean x64 install, static ASAR/resource validation, service lifecycle and uninstall without changing the system-proxy baseline. Packaged Electron/preload/tray/visible-window execution is separated into the manual `windows-gui-smoke` workflow and remains pending an interactive self-hosted Windows run. `.github/workflows/rc-upgrade-matrix.yml` plus the gated, bounded `scripts/rc-upgrade-matrix.ps1` implements N-1 → owner-approved unsigned RC → uninstall with checksum, profile-retention and no-mihomo assertions; it still needs two immutable release tags and a clean Windows run.)
...
- [~] Verify first-launch behavior with no profile and invalid profile. (Repository/service tests prove an empty directory is a valid zero-profile state, unreadable metadata/stale activation is ignored, and invalid YAML imports/activations do not create or replace the active profile. Packaged normal-launch visibility and idle behavior moved to the manual interactive `windows-gui-smoke` workflow and remain pending a qualifying Windows run.)
...
- [x] Decide and enforce release signing policy. (The owner explicitly chose not to purchase a certificate. Tag builds disable certificate discovery, require every release-owned installer/app/service executable to report Authenticode `NotSigned`, and record `not-signed-owner-approved` plus the expected Unknown publisher warning in immutable release evidence and notes.)
