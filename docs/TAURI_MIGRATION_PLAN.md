...
| 3B. Controller + streams | In progress | 已落地首片（`docs/tauri/phase3/README.md` 切片⑦）：kernel 监督器状态机（串行队列语义、start 幂等、stop 幂等+lastError 清除；DisabledKernelResolver 移植——start 记录 `failed`+逐字文案并传播 UNSUPPORTED，待 3C/3D 工件管线）+ 内核版本管理器（buildState 镜像 settings、固定 stable v1.19.30、预览版/Smart 标签、setChannel 尝试-回滚语义、specific 拒绝与 listVersions/install 的服务模式逐字守卫、`specificVersionsSupported=false` 与 Electron 服务模式一致）+ runtime 汇总（fail-safe 回退、活动档案名或品牌默认、external-ip 内核未运行恒 null）。10 通道 live。 | mihomo 控制器客户端 + 流事件（traffic/connections/logs）+ system-proxy + TUN 生命周期（3B/3D）；事件 emit 管道 |
