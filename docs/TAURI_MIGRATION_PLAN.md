...
| 3A. Local state | In progress | 已落地四片（`docs/tauri/phase3/README.md`）：① brand（同一 brand.config.json，无效即启动失败）+ app-info（Electron 词汇表）+ app-settings 存储移植（逐字节对齐）；② profiles 全链路（三件套布局、0600 原子写+fsync、名字唯一、注释保持型标量编辑、导入校验门、组序/Provider 目录解析、订阅 URL 脱敏 + keyring 源存储）；③ overrides 全链路（overrides.json 2 空格持久化、CRUD/move 重排序、+key/key+ 深合并引擎逐项 fail-open、预演/校验/回滚（lastKnownGood）、redactOverrideContent、中文文案逐字保留；`js` 覆写按文档化差异暂以警告跳过待 JS 沙箱切片）；④ 五个类型化单模型存储（core/geodata/dns/sniffer/tun-config：统一 ModelStore 持久化契约 `<file>.<ms>.tmp`+原子改名、get/set/preview 通道返回同构 envelope、coerce 逐字段移植、core storageVersion v2+0.9.0 端口互换迁移、TUN 9000→1500 迁移、DNS 预演脱敏、net 校验器移植；zod 模式层验证按文档化差异暂由 coerce 兜底）。`desktop_ipc` 分发表覆盖 46 通道。Rust `cargo test` **128 通过 0 警告**、typecheck 绿、TS 1905 通过/7 跳过（Linux ARM64 实测） | 剩余：usage history+文件日志、dialog/clipboard 等价物；`profiles:import-from-url` 等 3C 网络片；`get-provider-content` 等 3D 特权片；`inspect-active-config` 等有效文档合成片 |
