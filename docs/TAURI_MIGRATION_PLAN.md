...
| 3A. Local state | In progress | 已落地五片（`docs/tauri/phase3/README.md`）：① brand + app-info + app-settings；② profiles 全链路；③ overrides 全链路（`js` 覆写待 JS 沙箱切片）；④ 五个类型化单模型存储（core/geodata/dns/sniffer/tun-config，46 通道）；⑤ usage history（模型/有界存储 `<app-data>/usage-history/usage-history.json` 压缩 JSON+原子写+陈旧临时文件清理+0.8.5 旧计数重置、速率积分+非单调时钟守卫、连接计数、窗口聚合/排名/容量、4 通道；录制面待 3B/3D 内核流接入）。`desktop_ipc` 分发表覆盖 50 通道。Rust `cargo test` **139 通过 0 警告**、typecheck 绿、TS 1905 通过/7 跳过（Linux ARM64 实测） | 剩余：FileLogService（无 IPC 面，随 3B 内核流切片落地）、dialog/clipboard 等价物；`profiles:import-from-url` 等 3C 网络片；`get-provider-content` 等 3D 特权片；`inspect-active-config` 等有效文档合成片 |
