...
| 3A. Local state | In progress | 已落地：brand（Rust 解析同一 brand.config.json）+ app-info（Electron 词汇表映射）+ `app-settings.json` 存储的 Rust 移植（缺省/损坏隔离/原子写/2 空格+尾换行逐字节对齐 `src/main/app-settings/service.ts`，kernelEnabled 迁移语义、delayTestUrl 读写分离语义一致），Rust 单测 7 个设置用例 + 分发表 4 个用例 | profiles/source-metadata、overrides/DNS/sniffer/core/geodata/TUN 持久化、usage history、file logging、dialog/clipboard/safe-storage 等价物待后续轮次 |
