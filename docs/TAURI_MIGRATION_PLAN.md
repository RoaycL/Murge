...
| 3A. Local state | In progress | 已落地两片（`docs/tauri/phase3/README.md`）：① brand（Rust `include_str!` 同一 brand.config.json，无效即启动失败）+ app-info（Electron 词汇表）+ app-settings 存储移植（缺省/损坏隔离/原子写/2 空格+尾换行逐字节对齐，kernelEnabled 迁移、delayTestUrl 读时校验）；② profiles 全链路（`<id>.yaml`+`<id>.meta.json`+`active.json` 三件套、0600 原子写+fsync、名字唯一、sanitizeId、注释保持型标量编辑、导入校验门、组序/Provider 目录按文档序解析、订阅 URL 凭证脱敏 + OS 凭证库源存储 keyring、中文文案逐字保留）；`desktop_ipc` 分发表覆盖 21 通道，未就绪通道 fail-closed。Rust `cargo test` **70 通过 0 警告**、typecheck 绿、TS 1905 通过/7 跳过（Linux ARM64 实测） | 剩余：overrides/DNS/sniffer/core/geodata/TUN 持久化、usage history+文件日志、dialog/clipboard 等价物；`profiles:import-from-url`/`update-from-source` 等 3C 网络片；`get-provider-content` 等 3D 特权片；`inspect-active-config` 等有效文档合成片 |
