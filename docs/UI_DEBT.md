...
> **2026 自适应变更（有意偏离，已获所有者批准）：** 布局已从“固定卡片 + 留白”改为完全流式——页面壳 `max-width` 居中、网格 `1fr`/`auto-fill` 列、最小窗口放宽至 760×560。`UI_SPEC.md` 的视口条款与 `activity-ui-contract.test.ts` 已同步更新。934×672 仍是打开尺寸与截图验收基准；后续视觉验收需额外覆盖放大与 760px 最小宽度两个状态。**补充（2026-09-04，所有者指示）：活动页仪表盘在任何宽度下都保持双列，不做单列降级。**
