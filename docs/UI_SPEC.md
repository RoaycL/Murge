...
- 最小窗口：`760 × 560`（约可放下侧边栏 + 单列仪表盘）。

**自适应策略（2026 年修订，取代原“保持卡片尺寸并增加留白”）：**

布局采用完全流式。窗口大于基准视口时，网格列（`1fr` 轨道）与列表/表单页随容器拉伸，卡片行高保持规范值以维持比例；窗口缩小时，`auto-fill/minmax` 网格自动减少列数，仪表盘在 ≤960 px 时降级为单列，任何宽度下都不得出现横向滚动条。图表等视觉元素可随卡片宽度拉伸，但行高比例必须保持。
