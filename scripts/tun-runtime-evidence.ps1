...
try {
  ...
  $statusResp = Invoke-Pipe (@{ protocolVersion = 6; requestId = '100'; operation = 'status' } | ConvertTo-Json -Compress)
  Log "status      -> $statusResp"
  if ($statusResp -notmatch '"outcome":"stopped"') { throw "expected idle status, got: $statusResp" }

  $reconcileResp = Invoke-Pipe (@{ protocolVersion = 6; requestId = '101'; operation = 'reconcile' } | ConvertTo-Json -Compress)
  ...
  $unsafeReq = @{ protocolVersion = 6; requestId = '102'; operation = 'start'; sessionId = '00000000-0000-4000-8000-000000000001'; profile = $unsafeProfile; profileSha256 = $digest }
