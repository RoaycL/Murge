...
<template>
  <div class="page-shell connections-view">
    <header class="connections-header"><div><h1>连接</h1><p>{{ status === 'live' ? '实时' : status === 'loading' ? '连接中' : '已断开' }}</p></div><div class="connection-totals" aria-label="连接累计流量"><span><AppIcon name="upload" :size="14" />{{ formatBytes(summary?.uploadTotal ?? 0) }}</span><span><AppIcon name="download" :size="14" />{{ formatBytes(summary?.downloadTotal ?? 0) }}</span></div></header>
    ...
    <section class="connection-list full-width-list" :aria-label="view === 'active' ? '活动连接' : '已关闭连接'">
      <article v-for="connection in visibleConnections" :key="connection.id" class="connection-card" :class="{ selected: selectedId === connection.id }"><button type="button" class="connection-card-main" @click="store.select(connection.id)"><span class="process-icon"><AppIcon name="processes" :size="22" /></span><span class="connection-identity"><strong>{{ connection.metadata.process || '未知进程' }} <i>→</i> {{ endpoint(connection) }}</strong><small><b>{{ connection.metadata.network || 'TCP' }}</b><b>{{ connection.chains.join(' → ') || 'DIRECT' }}</b><b>↑ {{ formatBytes(connection.upload) }} ↓ {{ formatBytes(connection.download) }}</b></small></span><time>{{ elapsed(view === 'closed' ? connection.closedAt : connection.start) }}</time><AppIcon name="next" :size="15" /></button><button v-if="view === 'active'" type="button" class="connection-close" :disabled="closingIds.includes(connection.id)" aria-label="关闭连接" @click="store.close(connection.id)"><AppIcon name="close" :size="17" /></button></article>
