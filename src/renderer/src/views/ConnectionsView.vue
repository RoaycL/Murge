...
<template>
  <div class="page-shell connections-view">
    ...
    <div class="connections-toolbar"><label class="search-control"><AppIcon name="search" :size="15" /><input v-model="search" type="search" placeholder="进程、域名、IP 或策略" aria-label="筛选连接" /></label><AppSelect v-model="sort" :options="SORT_OPTIONS" label="连接排序" /><button v-if="view === 'active'" type="button" class="danger-button connection-toolbar-action" :disabled="closingMany || !visibleConnections.length" @click="confirmClose = true"><AppIcon name="close" :size="14" />{{ closingMany ? '正在关闭…' : '关闭全部' }}</button><button v-else type="button" class="secondary-button connection-toolbar-action" :disabled="!closedConnections.length" @click="store.clearClosed"><AppIcon name="delete" :size="14" />清空</button></div>
